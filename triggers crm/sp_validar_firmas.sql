...
SELECT firmas_satelite INTO @find_firmas_satelite FROM vtiger_contactdetails WHERE contactid=id_contactid LIMIT 1;

IF FOUND_ROWS()=0 THEN

	SELECT agenteid INTO @find_agenteid FROM vtiger_users WHERE id=id_contactid LIMIT 1;
    
    IF isnull(@find_agenteid) or (@find_agenteid="") THEN
		SELECT id INTO find_old_contactid FROM vtiger_users WHERE agenteid LIKE CONCAT("%",firma,"%") LIMIT 1;
		IF FOUND_ROWS()>0 THEN
...
ELSEIF isnull(@find_firmas_satelite) or (@find_firmas_satelite="") THEN
	...
	SELECT contactid INTO find_old_contactid FROM vtiger_contactdetails WHERE firmas_satelite LIKE CONCAT("%",firma,"%") LIMIT 1;
    IF FOUND_ROWS()>0 THEN
