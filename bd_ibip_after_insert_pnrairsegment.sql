...
FOR EACH ROW BEGIN
	...
	UPDATE vtigercrm600.vtiger_boletos 	SET status=NEW.emittedStatus 			WHERE boleto1 		LIKE CONCAT('%',NEW.ticketNumber,'%');
	UPDATE registro_boletos.boletos 	SET status_emission=NEW.emittedStatus 	WHERE ticketNumber 	LIKE CONCAT('%',NEW.ticketNumber,'%') AND sistemagds='Amadeus';
	UPDATE ifm_pnrticket 				SET emittedStatus=NEW.emittedStatus 	WHERE ticketNumber 	LIKE CONCAT('%',NEW.ticketNumber,'%');
